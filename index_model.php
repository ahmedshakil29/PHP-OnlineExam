...
class IndexModel
{
	...
	public function check_Exam_login($eid, $password)
	{
		$q = "SELECT * FROM quiz WHERE QuizID='" . $eid . "' and QuizPassword =" . $password;
		$r = mysqli_query($this->con, $q);
		if ($r == false) {
			return 0;
		} else {
	...
	{
		$q = "SELECT *,DepartmentName,courses.DepartmentID FROM courses LEFT OUTER JOIN department on courses.DepartmentID = department.DepartmentID ORDER BY CourseCode asc";
		$r = mysqli_query($this->con, $q);
		if ($r == false) {
			return 0;
		} else {
	...
	public function getStudentsBySectionsByTeacher($SectionID)
	{
		$q = "SELECT student_section.ID,section.SectionID,section.SectionName,users.UserID,users.UserName,users.UserEmail FROM `student_section` LEFT OUTER JOIN users on student_section.StudentID=users.UserID LEFT OUTER JOIN section on section.SectionID=student_section.SectionID where student_section.SectionID=" . $SectionID;
		$r = mysqli_query($this->con, $q);
		if ($r == false) {
			return 0;
		} else {
	...
	public function getAllTeacherSectionInfo($TeacherID)
	{
		$q = "select section.*,users.UserName,courses.CourseName from section LEFT OUTER JOIN  courses on courses.CourseID=section.CourseID LEFT OUTER JOIN users on section.TeacherID=users.UserID WHERE section.TeacherID=" . $TeacherID;
		$r = mysqli_query($this->con, $q);
		if ($r == false) {
			return 0;
		} else {
	...
	public function insert_quiz($QuizID, $QuizTitle, $TotalQuestion, $Right, $Wrong, $QuizTime, $QuizPassword, $Details,  $SectionID)
	{
		$q = "INSERT INTO `quiz`  VALUES ('" . $QuizID . "','" . $QuizTitle . "'," . $TotalQuestion . ", " . $Right . "," . $Wrong . "," . $QuizTime . ",'" . $QuizPassword . "','" . $Details . "', " . $SectionID . ", NOW() )";
		if (mysqli_query($this->con, $q)) {
			return 1;
		} else {
			return 0;
	...
	public function insert_feedback($StudentID, $Feedback, $SectionID, $s)
	{
		$q = "INSERT INTO `feedback`  VALUES (NULL,'" . $StudentID . "'," . $SectionID . ", '" . $Feedback . "','" . $s . "', NOW() )";
		if (mysqli_query($this->con, $q)) {
			return 1;
		} else {
			return 0;
	...
	// public function insert_question($QuizID, $QuestionID, $TotalQuestion, $Question, $ch, $i)
	// {
	// 	$q = "INSERT INTO questions VALUES  ('" . $QuizID . "','" . $QuestionID . "','" . $Question . "' ," . $ch . "," . $i . ")";
	// 	if (mysqli_query($this->con, $q)) {
	// 		return 1;
	// 	} else {
	// 		return 0;
	...
	{
		$q = "UPDATE `users` SET `Password`=' 1234 ' WHERE `UserID`=" . $UserID;
		if (mysqli_query($this->con, $q)) {
			return 1;
		} else {
			return 0;
	...
	public function update_profile($UserID, $UserName, $UserEmail, $Password, $Status)
	{
		$q = "UPDATE `users` SET `UserName`='" . $UserName . "', `UserEmail`='" . $UserEmail . "',  `Password`=" . $Password . " , `Status`=" . $Status . " WHERE `UserID`=" . $UserID;
		if (mysqli_query($this->con, $q)) {
			return 1;
		} else {
			return 0;
